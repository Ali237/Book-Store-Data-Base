        ...
        if user_input == 'y':
            ...
            while validinput:
                ...
                if is_valid_email(email):
                    break
                print("Invalid eamil please enter again\n") 
...
def view_profile(email, conn):
    ...
    ## Fetch and display user's borrowings
        ...
        WHERE member = ? AND end_date IS NULL 
                  OR (julianday(date('now')) - julianday(start_date) > 20);''', (email,))   
...
def search_books(cursor,email):
    ...
    try:
        ...
        if 1 <= selection <= len(all_books):
            book_to_borrow = all_books[selection - 1]
            if book_to_borrow[5] == 'Available':
                borrow_book(cursor, book_to_borrow[0],e)  # Pass book id if available
            else:
                print("Sorry, the selected book is not available for borrowing.")
        else:
            print("Invalid selection. Please enter a number corresponding to a book.")
    except ValueError:
        print("Invalid input. Please enter a number.")

# Function to handle borrowing a book
def borrow_book(cursor, book_id,e):
    today_date = date.today().strftime("%Y-%m-%d")
    # Insert borrowing record
    cursor.execute("INSERT INTO borrowings (book_id,member,start_date) VALUES (?,?,?)", (book_id,e, today_date))
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            search_books(conn.cursor(),email)
        elif choice == '4':
